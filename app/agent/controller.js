...
async function store(req, res, next) {
    try {
        ...
        if(payload.whatsapp)
            payload = {...payload, whatsapp: payload.whatsapp.replace(/[^0-9]/g, "")};

        if (req.file) {
            let tmp_path = req.file.path;
            let originalExt = req.file.originalname.split('.')[req.file.originalname.split('.').length - 1];
            let filename = req.file.filename + '.' + originalExt;
            let target_path = path.resolve(config.rootPath, `public/images/agents/${filename}`);
            ...
            src.on('end', async () => {
                try {
                    ...
                    if(error && error.name === 'ValidationError'){
                        return res.render('agent/view/form', {
                            error: 1, 
                            title: req.thisMenu ,
        ...
        if (req.file) {
            ...
            src.on('end', async () => {
                try {
                    ...
                    let currentImage = `${config.rootPath}/public/images/agents/${data.image_url}`;
